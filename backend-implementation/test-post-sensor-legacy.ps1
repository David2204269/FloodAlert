# Script de prueba para el endpoint POST /api/v1/data/sensor (formato legacy)
# Uso: .\test-post-sensor-legacy.ps1
...
if (Test-Path $envFile) {
    ...
    Get-Content $envFile | ForEach-Object {
        ...
        if ($_ -and $_ -notmatch '^\s*#' -and $_ -match '^\s*([^=]+?)\s*=\s*(.*?)\s*$') {
            ...
            # Remover comillas si las hay (dobles o simples)
...
Write-Host ""
Write-Host "🧪 Probando endpoint POST /api/v1/data/sensor (formato legacy)" -ForegroundColor Cyan
Write-Host "   Usando API Key: $($ApiKey.Substring(0, [Math]::Min(20, $ApiKey.Length)))..." -ForegroundColor Gray
Write-Host ""
